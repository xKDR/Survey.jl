...
@testset "glm.jl" begin
    ...
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(cnum~dnum),sd,Normal(),IdentityLink()).coefficients
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(cnum~dnum),sd,Normal(),InverseLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(cnum~dnum),sd,Normal(),LogLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(cnum~dnum),sd,Normal(),SqrtLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(cnum~dnum),sd,Gamma(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(cnum~dnum),sd,Gamma(),InverseLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(cnum~dnum),sd,Gamma(),LogLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(cnum~dnum),sd,Gamma(),SqrtLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(cnum~dnum),sd,Poisson(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(cnum~dnum),sd,Poisson(),InverseLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(cnum~dnum),sd,Poisson(),LogLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(cnum~dnum),sd,Poisson(),SqrtLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(growth~meals+ell+dnum),sd,Normal(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(meals~ell+growth),sd,Normal(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(meals~ell+growth),sd,Poisson(),InverseLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(meals~ell+growth),sd,Poisson(),LogLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(meals~ell+growth),sd,Poisson(),SqrtLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(full~ell+growth+meals),sd,Normal(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(full~ell+growth+meals),sd,Normal(),InverseLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(full~ell+growth+meals),sd,Normal(),LogLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(full~ell+growth+meals),sd,Normal(),SqrtLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(full~ell+growth+meals),sd,Gamma(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(full~ell+growth+meals),sd,Gamma(),InverseLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(full~ell+growth+meals),sd,Gamma(),LogLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(full~ell+growth+meals),sd,Gamma(),SqrtLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(full~ell+growth+meals),sd,Poisson(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(full~ell+growth+meals),sd,Poisson(),InverseLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(full~ell+growth+meals),sd,Poisson(),LogLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(full~ell+growth+meals),sd,Poisson(),SqrtLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(pcttest~api00+api99),sd,Normal(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(pcttest~api00+api99),sd,Normal(),InverseLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(pcttest~api00+api99),sd,Normal(),LogLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(pcttest~api00+api99),sd,Normal(),SqrtLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(pcttest~api00+api99),sd,Gamma(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(pcttest~api00+api99),sd,Gamma(),InverseLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(pcttest~api00+api99),sd,Gamma(),LogLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(pcttest~api00+api99),sd,Gamma(),SqrtLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(pcttest~api00+api99),sd,Poisson(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(pcttest~api00+api99),sd,Poisson(),InverseLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(pcttest~api00+api99),sd,Poisson(),LogLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(pcttest~api00+api99),sd,Poisson(),SqrtLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(growth~meals),sd,Normal(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(mobility~meals),sd,Normal(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(mobility~meals),sd,Normal(),InverseLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(mobility~meals),sd,Normal(),LogLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(mobility~meals),sd,Normal(),SqrtLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(mobility~meals),sd,Gamma(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(mobility~meals),sd,Gamma(),InverseLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(mobility~meals),sd,Gamma(),LogLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(mobility~meals),sd,Gamma(),SqrtLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(mobility~meals),sd,Poisson(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(mobility~meals),sd,Poisson(),InverseLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(mobility~meals),sd,Poisson(),LogLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apistrat.wts = genWts(apistrat,1)
    sd = design(data=apistrat,weights=:wts)
    jcoef = glm(@formula(mobility~meals),sd,Poisson(),SqrtLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    sd = design(data=apiclus1,weights=:wts)
    jcoef = glm(@formula(cnum~dnum),sd,Normal(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apiclus1.wts = genWts(apiclus1,1)
    sd = design(data=apiclus1,weights=:wts)
    jcoef = glm(@formula(meals~ell+growth),sd,Normal(),InverseLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apiclus1.wts = genWts(apiclus1,1)
    sd = design(data=apiclus1,weights=:wts)
    jcoef = glm(@formula(meals~ell+growth),sd,Normal(),LogLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apiclus1.wts = genWts(apiclus1,1)
    sd = design(data=apiclus1,weights=:wts)
    jcoef = glm(@formula(meals~ell+growth),sd,Normal(),SqrtLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apiclus1.wts = genWts(apiclus1,1)
    sd = design(data=apiclus1,weights=:wts)
    jcoef = glm(@formula(meals~ell+growth),sd,Gamma(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol

    rcoef = [0.03049724650374688, -0.00031040801568211357, -8.65111043794249e-6]
    apiclus1.wts = genWts(apiclus1,1)
    sd = design(data=apiclus1,weights=:wts)
    jcoef = glm(@formula(meals~ell+growth),sd,Gamma(),InverseLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apiclus1.wts = genWts(apiclus1,1)
    sd = design(data=apiclus1,weights=:wts)
    jcoef = glm(@formula(meals~ell+growth),sd,Gamma(),LogLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apiclus1.wts = genWts(apiclus1,1)
    sd = design(data=apiclus1,weights=:wts)
    jcoef = glm(@formula(meals~ell+growth),sd,Gamma(),SqrtLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    apiclus1.wts = genWts(apiclus1,1)
    sd = design(data=apiclus1,weights=:wts)
    jcoef = glm(@formula(meals~ell+growth),sd,Poisson(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    sd = design(data=apiclus2,weights=:wts)
    jcoef = glm(@formula(cnum~dnum),sd,Normal(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    sd = design(data=apipop,weights=:wts)
    jcoef = glm(@formula(cnum~dnum),sd,Normal(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
    ...
    sd = design(data=apisrs,weights=:wts)
    jcoef = glm(@formula(cnum~dnum),sd,Normal(),IdentityLink()).coefficients
    @test  maximum(abs.((jcoef-rcoef)./rcoef))<=rtol
